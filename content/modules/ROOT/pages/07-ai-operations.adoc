...
*Context note:* Most commands run on the *hub cluster* (`<hub> $`). Some verification steps in section 7A benefit from direct access to *gpu-cluster*. If you set up named contexts in xref:05-governance.adoc[Exercise 5], you already have `hub` and `standard-cluster`. Add `gpu-cluster` the same way:
...
. Reads the PlacementDecision to find gpu-cluster
. Creates an ArgoCD Application targeting gpu-cluster
. Deploys all manifests (namespace, buildconfig, deployment, service, route)
. The BuildConfig triggers a build on gpu-cluster from the git source

In the ArgoCD console, you should see the `ai-gpu-flask-gpu-cluster` application alongside any existing applications:

image::argocd-applications-overview.png[ArgoCD Applications Overview]

Drilling into the application shows the full resource tree -- namespace, deployment, build, service, and route all healthy:

image::argocd-app-detail-tree.png[ArgoCD Application Detail Tree]
...
OpenShift Lightspeed includes a built-in *Kubernetes MCP Server* (`openshift-mcp-server`) that runs as a sidecar container in the `lightspeed-app-server` pod. This server implements the https://modelcontextprotocol.io/[Model Context Protocol] and provides 14 read-only tools for querying live cluster resources.
...
image::lightspeed-before-mcp.png[Lightspeed before MCP -- hallucinated cluster names]
...
image::lightspeed-after-mcp.png[Lightspeed after MCP -- real cluster data]
