...
[NOTE]
====
The exercise depends on the ACM application deployed in the previous exercise (NOT the application deployed using ArgoCD). If the application is not available in your environment, run the next command to deploy it -
====
...
[NOTE]
====
Make sure that the `environment=production` label is associated with the managed cluster!
====
...
image::networkpolicy-status.png[networkpolicy-status]
