= Module 05: Governance Risk and Compliance
...
[NOTE]
====
This exercise depends on the ArgoCD-deployed webserver application from Exercise 4. The application should already be running in the `webserver-dev` namespace on `standard-cluster`. If the application is not running, go back to Exercise 4 and deploy the ApplicationSet first.
====

*Context note:* Most commands in this exercise run on the *hub cluster* (`<hub> $`). Some verification steps require access to *standard-cluster* (`<managed cluster> $`). Set up named contexts now so you can switch easily without re-logging in. If you haven't already obtained the credentials, retrieve them from the hub (see also xref:02-cluster-management.adoc[Exercise 2]):
...
image::policy-networkpolicy-status.png[policy-networkpolicy-status]
...
image::policy-limitrange-status.png[policy-limitrange-status]
...
. For this exercise, create a fork of the next GitHub repository - https://github.com/tosin2013/rhacm-workshop[https://github.com/tosin2013/rhacm-workshop]
+
As a result, you will have your own version of the repository - https://github.com/tosin2013/rhacm-workshop[https://github.com/<your-username>/rhacm-workshop]

. Afterwards, create a namespace on which you will deploy the RHACM resources (Use the namespace.yaml file in the forked repository) -
...
. Now, clone the official policy-collection GitHub repository to your machine. The repository contains a binary named *deploy.sh*. The binary is used to associate policies in a GitHub repository to a running Red Hat Advanced Cluster Management for Kubernetes cluster.
...
[NOTE]
====
If you are already logged in as a non-kubeadmin user with cluster-admin privileges (e.g. `admin`), skip steps 4.a through 4.d entirely. In step 5 below, replace `workshop-admin` with your current username (run `oc whoami` to check). Do NOT run the `oc replace -f oauth.yaml` command if your cluster already has an identity provider configured -- it will overwrite your existing provider.
====
...
[TIP]
====
Install httpd tools if missing.
====
...
. You can now deploy the policies from your forked repository to Advanced Cluster Management.
...
. Make sure that the policies are deployed in the *Governance* -> *Policies* tab in the Advanced Cluster Management for Kubernetes console.

image::policies-overview.png[policies-overview]


. Edit the LimitRange policy in https://github.com/tosin2013/rhacm-workshop/blob/master/05.Governance-Risk-Compliance/exercise/exercise-policies/limitrange-policy.yaml[https://github.com/<your-username>/rhacm-workshop/blob/master/05.Governance-Risk-Compliance/exercise/exercise-policies/limitrange-policy.yaml]. Change the default container limit from 512Mi to 1024Mi.

. Make sure that you commit, and push the change to your fork.

. Log into managed cluster. Make sure that the change in GitHub was applied to the LimitRange resource.
...
In this section you will use RHACM's templating mechanism for governance policies. In this scenario, you will create an RHACM application. The application deploys a mariadb database and a Prometheus exporter (https://github.com/prometheus/mysqld_exporter[mysqld-exporter]) that connects to the database and exports metrics.
...
In this scenario, you will pass two templated variables to the mysqld-exporter deployment using a dedicated ConfigMap resource. The variables are merged into a single _connection string_ that the exporter uses to connect to the mariadb database.
...
To further understand the structure of the application, go over the link:exercise/exercise-application[application resources]. All of the application resources are present in this directory besides the ConfigMap resource which is created using a templated policy.

The next link:exercise/exercise-templates/metrics-configmap.yaml[templated policy] is used to create the ConfigMap resource that the exporter uses as a connection string - 
