...
Now, import both `local-cluster` and `standard-cluster` into the ManagedClusterSet resource. Importation will be done by adding the `cluster.open-cluster-management.io/clusterset: all-clusters` label to each ManagedCluster resource -
...
[source,bash]
----
<standard-cluster> $ oc get route -n webserver-dev

NAME        HOST/PORT                             PATH                SERVICES    PORT       TERMINATION   WILDCARD
webserver   webserver-webserver-dev.apps.<FQDN>   /application.html    webserver   8080-tcp   edge          None
----

TIP: If you don't have a kubeconfig context for `standard-cluster`, you can retrieve the route from the hub using RHACM search or the ArgoCD UI.
